...
local function renderTarget(t, isTerm)
  ...
  if dispE ~= nil or dispCap ~= nil then
    ...
    local eta = etaText(dispE, dispCap, state.rate)
    if eta then
      line("ETA:      " .. eta)
    end
    if state.trueE then
...
-- hand the terminal back (we drive monitors), then run the standard updater
-- (download + reboot into our role). Sleds keep their own "sledctl" channel.
