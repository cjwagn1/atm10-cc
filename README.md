...
1. **Every program is tested before it ever touches the server** — in a
   ...
   (vendored under `vendor/`). **139 tests** across two suites
   (79 telemetry + 60 Project Sled), written TDD-style (red → green) —
   including a kill-sweep that reboots the sled turtle at every half-second
   of its work cycle and demands convergence.
...
```bash
toolchain/lua-5.2.4/src/lua tests/run_tests.lua       # 79 tests (telemetry + base control)
...
tests/       run_tests.lua (79) + run_sled_tests.lua (60; Project Sled)
